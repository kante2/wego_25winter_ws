// gate_lidar_fusion_node.cpp
// Lidar 기반 게이트 인식 + 카메라 연동 Fusion 노드
// 입력:
//   - /scan (LaserScan)
//   - /perception/gate_yellow_detected (카메라 노란 게이트 검출 여부, Bool)
// 출력:
//   - /gate_points (게이트 클러스터 PointCloud2)
//   - /gate_cluster_marker (Marker)
//   - /perception/gate_lidar   (라이다 기반 게이트 DOWN 감지 Bool)
//   - /perception/gate_fusion  (카메라 AND 라이다 Fusion Bool)
...
// 카메라 gate yellow 검출 플래그
static bool g_cam_gate_yellow = false;
...
// 카메라에서 오는 /perception/gate_yellow_detected 구독
static void camGateCallback(const std_msgs::BoolConstPtr& msg)
{
  g_cam_gate_yellow = msg->data;
}
...
static ros::Publisher  g_pub_gate_lidar;    // lidar 기반 gate 여부
static ros::Publisher  g_pub_gate_fusion;   // 카메라 + 라이다 fusion
...
static void scanCallback(const sensor_msgs::LaserScanConstPtr& scan)
{
  ...
  // --- 포인트 필터링: 13cm ~ 50cm, 전방 기준 [g_front_min_deg, g_front_max_deg] ---
  ...
  // 라이다 기준으로 실제 "STOP해야 하는" 상태인지 판단
  bool lidar_detected =
      gate_down_final &&
      std::isfinite(gate_dist) &&
      (gate_dist <= g_gate_stop_dist);

  // --- (5) 결과 퍼블리시 (Cloud / Marker / Lidar Bool / Fusion Bool) ---
  publishCloud(gate_cluster, scan->header);
  publishMarker(gate_cluster, scan->header);

  // 1) 라이다 전용 토픽
  std_msgs::Bool lidar_msg;
  lidar_msg.data = lidar_detected;
  g_pub_gate_lidar.publish(lidar_msg);

  // 2) 카메라 + 라이다 Fusion
  bool fusion_detected = lidar_detected && g_cam_gate_yellow;
  std_msgs::Bool fusion_msg;
  fusion_msg.data = fusion_detected;
  g_pub_gate_fusion.publish(fusion_msg);

  ROS_INFO_THROTTLE(0.5,
                    "[gate_fusion] lidar=%d, cam=%d -> fusion=%d",
                    lidar_detected ? 1 : 0,
                    g_cam_gate_yellow ? 1 : 0,
                    fusion_detected ? 1 : 0);
}
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber enable_sub =
      nh.subscribe<std_msgs::Bool>(g_enable_topic, 1, enableCallback);

  // 카메라 gate yellow 구독
  ros::Subscriber cam_sub =
      nh.subscribe<std_msgs::Bool>("/perception/gate_yellow_detected", 1, camGateCallback);
  ...
  // 라이다 결과: /perception/gate_lidar
  g_pub_gate_lidar =
      nh.advertise<std_msgs::Bool>("/perception/gate_lidar", 1);

  // Fusion 결과: /perception/gate_fusion
  g_pub_gate_fusion =
      nh.advertise<std_msgs::Bool>("/perception/gate_fusion", 1);

  ROS_INFO("gate_node (Lidar + Camera Fusion) started.");
  ros::spin();
  return 0;
}
