...
class LaneFollow:
    ...
    def yellow_color_filter_hsv(self, img):
        ...
        if self.yellow_detected:
            rospy.loginfo_throttle(2.0, f"[YellowDetect] Yellow lane detected! Ratio: {yellow_ratio:.3f}")
...
# callback 
    ...
    def scan_callback(self, msg):
        ...
        if min_front_distance < self.obstacle_stop_distance:
            ...
            rospy.logwarn_throttle(1.0, 
                f"[Caution] Obstacle detected! Distance: {min_front_distance:.3f}m")
            self.stop_flag = False  # 회피 가능하므로 정지는 하지 않음
        else:
            self.stop_flag = False
